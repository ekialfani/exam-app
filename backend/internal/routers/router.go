...
func StartServer() *gin.Engine {
	...
	examRouter := router.Group("/exams")
	{
		...
		examRouter.GET("/reports/", middlewares.AdminAuthorization(), controllers.GetAllExamReports)
		examRouter.GET("/reports/:examId", middlewares.AdminAuthorization(), middlewares.ExamAuthorization(), controllers.GetExamReportByExamId)
		examRouter.GET("/shuffled-questions/:examId", controllers.GetExamWithShuffledQuestions)
	...
	studentRouter := router.Group("/students")
	{
		...
		studentRouter.GET("/:studentId", controllers.GetStudentById)
		studentRouter.PUT("/:studentId", middlewares.StudentAuthorization(), controllers.UpdateStudent)
		studentRouter.PATCH("/:studentId", middlewares.StudentAuthorization(), controllers.UpdateStudentPassword)
	...
	examAssignmentRouter := router.Group("/exam-assignments")
	{
		...
		examAssignmentRouter.DELETE("/:examId", controllers.DeleteExamAssignment)
	}
	...
	completedExamRouter := router.Group("completed-exams")
	{
		...
		completedExamRouter.GET("/:examId", controllers.GetCompletedExamDetail)
	}
